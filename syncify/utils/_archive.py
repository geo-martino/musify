...
class Misc:
    ...
    def titlecase_folders(self, folders: dict, **kwargs) -> None:
        ...
        for folder, tracks in folders.items():
            ...
            album = titlecase(
                track['album'].replace(
                    '"', "'"), callback=self.caps).strip()

            # sanitise folder name from illegal characters and title case
            # string
            folder_old = folder
            folder = track['album'].replace(
                ' / ', ' - ').replace('/', ' - ').replace(': ', ' - ').replace(':', '-')
            folder = titlecase(folder.replace('"', "'"),
                               callback=self.caps).strip()
            ...
            if track['album'] != album:
                ...
                # print old and new album names and folder names and ask user
                # to confirm
    ...
    def titlecase_files(
        ...
        for folder, tracks in folders.items():
            ...
            if run or not prefix_start or (
                    prefix_start and prefix_start in folder):
                run = True
            else:
                continue
            ...
            for track in tracks:
                ...
                if title not in filename.strip() and title != filename.capitalize():
                    ...
                    if track['track'] is not None:  # if track number in file tags
                        ...
                        track = f"0{track['track']}" if track['track'] // 10 == 0 else str(
                            track['track'])

                        if track.strip() in filename[:3].strip():
                            # add track number to filename if already in filename
                            text = f'{track} - {title}'
    ...
    def tags_from_filename(self, folders: dict, no_rename: list = None,
        ...
        for folder, tracks in folders.items():
            # check if folder name matches start folder if given
            if run or not prefix_start or (
                    prefix_start and folder.lower().startswith(prefix_start)):
                run = True
            else:
                continue
            ...
            for track in tracks:
                ...
                # open file, extract file extension, and define tag names for
                # this file type
                        ...
                        for tag_var in tags[name]:
                            ...
                            if tag_var in file_data and re.sub(
                                ...
                                if ext == '.flac':
                                    ...
                                    file_data[tag_var] = getattr(
                                        mutagen.id3, tag_var)(3, tag)
                                elif ext == '.m4a':
                                    if name == 'track':
                                        file_data[tag_var] = [
                                            (int(tag), len(tracks))]
                                    ...
                                    file_data[tag_var] = mutagen.asf.ASFUnicodeAttribute(
                                        tag)
                ...
                if changed:  # if a tag has been modified
                    ...
                    if 'track' in changed:  # if track number has been changed
                        # if the only change is adding a leading zero,
                        # track_match = True
                        if str(d['track'])[0] == '0':
                            track_match = True
                    ...
                    if 'title' in changed:  # if title has been changed
                        # if only case changes have occurred, title_match =
                        # True
                        title_match = d['title'].lower().strip() in str(
                            changed['title']).lower().strip()
                    else:  # True if title has not been changed
                        title_match = True

                    if 'album' in changed:  # if album has been changed
                        # if only case changes have occurred, album_match =
                        # True
                        album_match = d['album'].lower().strip() in str(
                            changed['album']).lower().strip()
                    else:  # True if album has not been changed
                        album_match = True

                    if all([title_match, album_match, track_match]):
                        # do not prompt user for confirmation if all above
                        # conditions are True
                        skip = False
