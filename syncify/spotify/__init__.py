from typing import Optional, List, Set, Union, MutableMapping, Any, Collection
...
def get_id_type(value: str) -> IDType:
    ...
    :exception ValueError: Raised when the function cannot determine the ID type of the input ``value``.
    """
...
def convert(
    ...
    :exception ValueError: Raised when the function cannot determine the item type of the input ``value``.
    """
...
def extract_ids(values: APIMethodInputType, kind: Optional[ItemType] = None) -> List[str]:
    ...
    raise ValueError(f"Could not extract IDs. Input data not recognised: {type(values)}")
