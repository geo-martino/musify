...
def report_library_differences(source: Library, compare: Library) -> dict[str, dict[str, tuple[Item, ...]]]:
    """
    ...
    max_width = get_max_width(source.playlists.keys())
...
def report_missing_tags(
    ...
    for collection, result in missing.items():
        logger.report(f"\33[1;91m -> {collection.name} \33[0m")
        for item, tags in result.items():
            name = align_and_truncate(item.name, max_width=max_width)
            logger.report(f"\33[96m{name} \33[0m| \33[93m{', '.join(tags)} \33[0m")
        logger.print(REPORT)
