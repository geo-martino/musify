...
class Syncify(Data, Spotify):
    ...
    def load_all_local(self, ex_playlists=None, ex_folders=None, in_folders=None, refresh=False):
        ...
        all_pl = self.get_all_metadata(ex_playlists=ex_playlists, ex_folders=ex_folders,
                                       in_folders=in_folders, verbose=self._verbose)
    ...
    def load_all_spotify(self, ex_playlists=None, ex_folders=None, in_folders=None):
        ...
        self.all_spotify_metadata = self.get_tracks_metadata(uri_list, self._headers, self._verbose)
    ...
    def load_spotify(self, in_playlists=None):
        ...
        self.spotify_metadata = self.get_playlists_metadata(self._headers, in_playlists, self._verbose)
        if self._verbose:
            print('\n', '-' * 88, '\n', sep='')
    ...
    def update_uri_from_spotify_playlists(self):
        ...
        uris = self.update_uris_from_playlists(self.m3u_metadata, self.spotify_metadata, self._verbose)
    ...
    def search_m3u_to_spotify(self, quick_load=False, refresh=False):
        ...
        if not quick_load:
            ...
            added, not_found, searched = self.search_all(self.m3u_metadata, self._headers,
                                                         force=refresh, verbose=self._verbose)
        ...
        updated = self.update_playlists(added, self.spotify_metadata, self._headers, self._verbose)
    ...
    def differences(self):
        ...
        self.spotify_metadata = self.get_playlists_metadata(self._headers, m3u, self._verbose)
        extra, missing = self.get_report(m3u, self.spotify_metadata, self._verbose)
    ...
    def update_artwork(self, album_prefix=None, replace=False):
        ...
        spotify_uri = self.get_tracks_metadata(list(local_uri.keys()), self._headers, verbose=self._verbose)
    ...
    def get_missing_uri(self, ex_playlists=False, quick_load=False, import_uri=True, drop=True, null_folders=None,
        ...
        if quick_load:  # set variables for quick_load
            ...
            found, not_found, searched = self.search_all(folders, self._headers, 'albums', add_back, self._verbose)
        ...
        self.check_tracks(found, self._headers, self.URI_FILENAME, verbose=self._verbose)
        ...
        if tags and found:
            ...
            spotify_metadata = self.get_tracks_metadata(uri_list, self._headers)
    ...
    def spotify_to_tag(self, tags, metadata=None, reduce=True, refresh=False):
        ...
        self.auth(lines=False, verbose=True)
        metadata = self.get_tracks_metadata(get_tags.keys(), self._headers, verbose=self._verbose)
