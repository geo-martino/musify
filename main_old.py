...
class Syncify(Data, Spotify):
    ...
    def __init__(
        ...
        self.BASE_AUTH = base_auth if base_auth else os.environ.get(
            'BASE_AUTH')
        ...
        self.CLIENT_SECRET = c_secret if c_secret else os.environ.get(
            'CLIENT_SECRET')
        ...
        self.OTHER_PATHS = {
            k: path for k,
            path in self.OTHER_PATHS.items() if len(path) > 1}

        # Get this system's associated paths, and store other system's paths
        if sys.platform == "win32":
            music_path = music_path if music_path else win_path
            self.WIN_PATH = normpath(
                music_path) if music_path else self.OTHER_PATHS.get('win')
            self.MUSIC_PATH = self.WIN_PATH

            self.LIN_PATH = normpath(
                lin_path) if lin_path else self.OTHER_PATHS.get('lin')
            self.MAC_PATH = normpath(
                mac_path) if mac_path else self.OTHER_PATHS.get('mac')

            self.OTHER_PATHS = [
                path for path in [
                    self.LIN_PATH,
                    self.MAC_PATH] if path]
        elif sys.platform == "linux":
            music_path = music_path if music_path else lin_path
            self.LIN_PATH = normpath(
                music_path) if music_path else self.OTHER_PATHS.get('lin')
            self.MUSIC_PATH = self.LIN_PATH

            self.WIN_PATH = normpath(
                win_path) if win_path else self.OTHER_PATHS.get('win')
            self.MAC_PATH = normpath(
                mac_path) if mac_path else self.OTHER_PATHS.get('mac')
            ...
            self.MAC_PATH = normpath(
                music_path) if music_path else self.OTHER_PATHS.get('mac')
            ...
            self.OTHER_PATHS = [
                path for path in [
                    self.WIN_PATH,
                    self.LIN_PATH] if path]

        # Build full path to playlist folder from this system's music path
        self.PLAYLISTS = playlists if playlists else os.environ.get(
            'PLAYLISTS')
        ...
        self.DATA_PATH = normpath(data) if data else normpath(
        ...
        self.URI_FILENAME = uri_file if uri_file else os.environ.get(
            'URI_FILENAME')
        self.TOKEN_FILENAME = token_file if token_file else os.environ.get(
            'TOKEN_FILENAME')
        ...
        if auth:
            self._headers = self.auth(verbose)
    ...
    def set_env(self, current_state=True, **kwargs):
        ...
        if exists('.env'):  # load stored environment variables if they exist
            with open('.env', 'r') as file:
                env = {line.rstrip().split('=')[0]: line.rstrip().split('=')[
                    1] for line in file}
        else:
            env = {}
        ...
        if current_state:  # update variables from current object
            env.update({var: getattr(self, var)
                       for var in env_vars if var in self.__dict__ and getattr(self, var)})
        ...
        return {
            var: env[var] for var in env_vars if var in self.__dict__ and getattr(
                self,
                var)}
    ...
    def load_all_local(
        ...
        :return: self.
        """
        all_pl = self.get_all_metadata(
            ex_playlists=ex_playlists,
            ex_folders=ex_folders,
    ...
    def load_all_spotify(
        ...
        """
        # authorise
        self._headers = self.auth(lines=False, verbose=True)
        ...
        uri_list = [track['uri'] for tracks in self.all_metadata.values()
                    for track in tracks if 'uri' in track]
        self.all_spotify_metadata = self.get_tracks_metadata(
            uri_list, self._headers, self._verbose)
    ...
    def load_m3u(self, in_playlists=None):
        ...
        :return: self.
        """
        m3u = self.get_m3u_metadata(in_playlists, self._verbose)
    ...
    def load_spotify(self, in_playlists=None):
        ...
        if in_playlists and 'm3u' in in_playlists:
            in_playlists = [
                splitext(playlist)[0] for playlist in os.listdir(
                    self.PLAYLISTS_PATH)]

        # get Spotify metadata
        self.spotify_metadata = self.get_playlists_metadata(
            self._headers, in_playlists, self._verbose)
        if self._verbose:
            print('\n', '-' * 88, '\n', sep='')
    ...
    def update_uri_from_spotify_playlists(self):
        ...
        uris = self.update_uris_from_playlists(
            self.m3u_metadata, self.spotify_metadata, self._verbose)
    ...
    def search_m3u_to_spotify(self, quick_load=False, refresh=False):
        ...
        if not quick_load:
            ...
            if refresh:  # clear playlists
                ...
                for name, playlist in self.spotify_metadata.items():
                    self.clear_from_playlist(playlist, self._headers)
                    self.spotify_metadata[name]['tracks'] = []
                print('\33[92m', 'Done', '\33[0m')

            # search for missing URIs
            added, not_found, searched = self.search_all(
                self.m3u_metadata, self._headers, force=refresh, verbose=self._verbose)
        ...
        updated = self.update_playlists(
            added, self.spotify_metadata, self._headers, self._verbose)
        if updated and self._verbose:
            print('\n', '-' * 88, '\n', sep='')

        # save json files, update local URIs, update this object's stored
        # Spotify metadata
    ...
    def differences(self):
        ...
        self.spotify_metadata = self.get_playlists_metadata(
            self._headers, m3u, self._verbose)
        extra, missing = self.get_report(
            m3u, self.spotify_metadata, self._verbose)
    ...
    def update_artwork(self, album_prefix=None, replace=False):
        ...
        if not replace:
            local_uri = {
                uri: track for uri,
                track in local_uri.items() if not track['has_image']}
        spotify_uri = self.get_tracks_metadata(
            list(local_uri.keys()), self._headers, verbose=self._verbose)
    ...
    def get_missing_uri(
        ...
        if quick_load:  # load from last search
            ...
            if self.all_metadata is None:  # load metadata if not yet done
                self.all_metadata = self.get_all_metadata(
                    ex_playlists=ex_playlists, verbose=False)
        ...
        if import_uri:  # import locally stored URIs
            ...
            if drop:  # drop files with no URIs
                folders = {
                    folder: [
                        track for track in tracks if 'uri' in track] for folder,
                    tracks in folders.items()}
        ...
        if quick_load:  # set variables for quick_load
            ...
            found, not_found, searched = self.search_all(
                folders, self._headers, 'albums', add_back, self._verbose)

        if searched and self._verbose:
            print('\n', '-' * 88, '\n', sep='')
        ...
        if not quick_load and (
                null_folders or searched):  # export URIs if updating has occurred
        ...
        if tags and found:
            ...
            uri_list = [song['uri']
                        for songs in found.values() for song in songs]
            spotify_metadata = self.get_tracks_metadata(
                uri_list, self._headers)
    ...
    def spotify_to_tag(self, tags, metadata=None, reduce=True, refresh=False):
        ...
        if reduce:
            for folder, songs in self.all_metadata.items():
                for song in songs:
                    if 'uri' not in song:
                        continue
                    if not ('.m4a' in song['path'] and any(
                            [t in tags for t in ['bpm', 'key']])):
                        if any([song[tag] is None and song['uri']
                               for tag in tags if tag in song]):
        ...
        self.auth(lines=False, verbose=True)
        metadata = self.get_tracks_metadata(
            get_tags.keys(), self._headers, verbose=self._verbose)
        ...
        self.update_tags(get_tags, metadata, refresh, verbose=self._verbose)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == 'update':  # run functions for updating Spotify playlists with new songs
        main.auth(**{k: v for k, v in {**kwargs}.items()
                  if k in main.auth.__code__.co_varnames})
        main.update_uri_from_spotify_playlists(**{k: v for k, v in {**kwargs}.items(
        ) if k in main.update_uri_from_spotify_playlists.__code__.co_varnames})
        main.search_m3u_to_spotify(
            refresh=False, **{
                k: v for k, v in {
                    **kwargs}.items() if k in main.search_m3u_to_spotify.__code__.co_varnames})
        main.differences(**{k: v for k, v in {**kwargs}.items()
                         if k in main.differences.__code__.co_varnames})

    # run functions for clearing and updating Spotify playlists with new songs
    ...
    # run functions to produce report on differences between local and Spotify
        ...
        main.auth(**{k: v for k, v in {**kwargs}.items()
                  if k in main.auth.__code__.co_varnames})
        main.update_artwork(**{k: v for k, v in {**kwargs}.items()
                            if k in main.update_artwork.__code__.co_varnames})

    # run functions to produce report on local songs with missing artwork
    elif sys.argv[1] == 'missing_artwork':
        main.load_all_local()
        report = main.missing_tags(
            main.all_metadata,
            tags=['has_image'],
            kind='album')
        main.save_json(report, 'missing_artwork')

    # run functions to extract all embedded images from locally stored songs
    elif sys.argv[1] == 'extract_local':
        main.load_all_local()
        main.extract_images(
            main.all_metadata, kind='local', foldername='local', **{
                k: v for k, v in {
                    **kwargs}.items() if k in main.extract_images.__code__.co_varnames})

    # run functions to extract all artwork from locally listed Spotify URIs
    elif sys.argv[1] == 'extract_spotify':
        main.auth(**{k: v for k, v in {**kwargs}.items()
                  if k in main.auth.__code__.co_varnames})
        spotify = main.load_spotify()
        main.extract_images(
            spotify, kind='spotify', foldername='spotify', **{
                k: v for k, v in {
                    **kwargs}.items() if k in main.extract_images.__code__.co_varnames})

    # run functions to search for and check associated URIs for all local songs
        ...
        null = [
            ...
            'Maturity (Instrumentals)',
            'Miss Saigon - OLC (Act 1)',
            'Miss Saigon - OLC (Act 1)',
            ...
            'Safe (Extras)',
            'Safe The Second',
            'Safe The Second (Extras)',
            'Z',
            'Resting State']
        tags = [
            'bpm',
            'key',
            'uri'] if 'tags' not in kwargs else kwargs.pop('tags')
        main.get_missing_uri(
            import_uri=True, null_folders=null, drop=False, tags=tags, **{
                k: v for k, v in {
                    **kwargs}.items() if k in main.get_missing_uri.__code__.co_varnames})

    # run functions to just check all already associated URIs with no search
    elif sys.argv[1] == 'simplecheck':
        URIs = main.load_json(main.URI_FILENAME)
        main.auth(lines=False, verbose=True)
        main.check_tracks(URIs, main.headers)

    # run functions to produce report on local songs with missing tags
    elif sys.argv[1] == 'missing_tags':
        main.load_all_local()
        tags = ['title', 'artist', 'album', 'track', 'year',
                'genre'] if 'tags' not in kwargs else kwargs.pop('tags')
        ignore = [
            'Backings',
            'Backings NSFW',
            'Old Ideas',
        ...
        tags = [
            ...
            'key',
            'uri'] if 'tags' not in kwargs else kwargs.pop('tags')
        main.spotify_to_tag(tags, **{k: v for k, v in {**kwargs}.items()
                            if k in main.spotify_to_tag.__code__.co_varnames})
