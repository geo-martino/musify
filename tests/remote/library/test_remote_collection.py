...
class RemotePlaylistTester(RemoteCollectionTester, PlaylistTester, metaclass=ABCMeta):
    ...
    ###########################################################################
    @abstractmethod
    def sync_mock(self, *args, **kwargs) -> RemoteMock:
        """
        Yields a requests_mock setup to return valid responses for the current remote source 
        when testing sync functionality as a pytest.fixture
        """
        raise NotImplementedError
    ...
    @staticmethod
    @abstractmethod
    def get_sync_uris(url: str, sync_mock: RemoteMock) -> tuple[list[str], list[str]]:
        """Return tuple of lists of URIs added and URIs cleared when applying sync operations"""
        raise NotImplementedError
    
    @staticmethod
    def assert_playlist_loaded(sync_playlist: RemotePlaylist, sync_mock: RemoteMock, count: int = 1) -> None:
        """Assert the given playlist was fully reloaded through GET requests ``count`` number of times"""
        pages = sync_mock.calculate_pages_from_response(sync_playlist.response)

        requests = sync_mock.get_requests(url=sync_playlist.url, method="GET")
        requests += sync_mock.get_requests(url=sync_playlist.url + "/tracks", method="GET")
        
        assert len(requests) == pages * count

    @staticmethod
    def test_sync_dry_run(sync_playlist: RemotePlaylist, sync_items: list[RemoteTrack], sync_mock: RemoteMock):
        ...
        assert len(sync_mock.request_history) == 0
        ...
        assert len(sync_mock.request_history) == 0  # reload does not happen on dry_run
    ...
    def test_sync_reload(self, sync_playlist: RemotePlaylist, sync_items: list[RemoteTrack], sync_mock: RemoteMock):
        ...
        self.assert_playlist_loaded(sync_playlist=sync_playlist, sync_mock=sync_mock, count=2)

    def test_sync_new(self, sync_playlist: RemotePlaylist, sync_items: list[RemoteTrack], sync_mock: RemoteMock):
        ...
        uri_add, uri_clear = self.get_sync_uris(url=sync_playlist.url, sync_mock=sync_mock)
        assert uri_add == [track.uri for track in sync_items]
        assert uri_clear == []

        # 1 for skip dupes check on add to playlist
        self.assert_playlist_loaded(sync_playlist=sync_playlist, sync_mock=sync_mock, count=1)

    def test_sync_refresh(self, sync_playlist: RemotePlaylist, sync_items: list[RemoteTrack], sync_mock: RemoteMock):
        start = len(sync_playlist)
    ...
    def test_sync(self, sync_playlist: RemotePlaylist, sync_items: list[RemoteTrack], sync_mock: RemoteMock):
        sync_items_extended = sync_items + sync_playlist[:10]
