...
def test_get_id_type(spotify_wrangler: SpotifyDataWrangler):
    ...
    assert spotify_wrangler.get_id_type(f"spotify:show:{random_id()}") == IDType.URI
    assert spotify_wrangler.get_id_type(f"{URL_API}/{random_id()}") == IDType.URL
    assert spotify_wrangler.get_id_type(f"{URL_EXT}/{random_id()}") == IDType.URL_EXT
...
def test_validate_id_type(spotify_wrangler: SpotifyDataWrangler):
    ...
    assert spotify_wrangler.validate_id_type(f"spotify:show:{random_id()}")
    assert spotify_wrangler.validate_id_type(f"{URL_API}/{random_id()}")
    assert spotify_wrangler.validate_id_type(f"{URL_EXT}/{random_id()}")

    assert spotify_wrangler.validate_id_type(random_id(), kind=IDType.ID)
    assert spotify_wrangler.validate_id_type(f"spotify:show:{random_id()}", kind=IDType.URI)
    assert spotify_wrangler.validate_id_type(f"{URL_API}/{random_id()}", kind=IDType.URL)
    assert spotify_wrangler.validate_id_type(f"{URL_EXT}/{random_id()}", kind=IDType.URL_EXT)

    assert not spotify_wrangler.validate_id_type(random_id(), kind=IDType.URL)
    assert not spotify_wrangler.validate_id_type(f"spotify:show:{random_id()}", kind=IDType.URL_EXT)


# noinspection SpellCheckingInspection
def test_get_item_type(spotify_wrangler: SpotifyDataWrangler):
    assert spotify_wrangler.get_item_type(f"spotify:playlist:{random_id()}") == ObjectType.PLAYLIST
    assert spotify_wrangler.get_item_type(f"spotify:TRACK:{random_id()}") == ObjectType.TRACK
    assert spotify_wrangler.get_item_type(f"spotify:ALBUM:{random_id()}") == ObjectType.ALBUM
    assert spotify_wrangler.get_item_type(f"spotify:artist:{random_id()}") == ObjectType.ARTIST
    assert spotify_wrangler.get_item_type("spotify:user:ausername") == ObjectType.USER
    assert spotify_wrangler.get_item_type(f"spotify:show:{random_id()}") == ObjectType.SHOW
    assert spotify_wrangler.get_item_type(f"spotify:episode:{random_id()}") == ObjectType.EPISODE
    assert spotify_wrangler.get_item_type(f"spotify:audiobook:{random_id()}") == ObjectType.AUDIOBOOK
    assert spotify_wrangler.get_item_type(f"spotify:chapter:{random_id()}") == ObjectType.CHAPTER

    assert spotify_wrangler.get_item_type(f"{URL_API}/playlists/{random_id()}/followers") == ObjectType.PLAYLIST
    assert spotify_wrangler.get_item_type(f"{URL_EXT}/TRACKS/{random_id()}") == ObjectType.TRACK
    assert spotify_wrangler.get_item_type(f"{URL_API}/ALBUMS/{random_id()}") == ObjectType.ALBUM
    assert spotify_wrangler.get_item_type(f"{URL_EXT}/artists/{random_id()}") == ObjectType.ARTIST
    assert spotify_wrangler.get_item_type(f"{URL_EXT}/users/ausername") == ObjectType.USER
    assert spotify_wrangler.get_item_type(f"{URL_API}/shows/{random_id()}/episodes") == ObjectType.SHOW
    assert spotify_wrangler.get_item_type(f"{URL_EXT}/episodes/{random_id()}") == ObjectType.EPISODE
    assert spotify_wrangler.get_item_type(f"{URL_API}/audiobooks/{random_id()}/chapters") == ObjectType.AUDIOBOOK
    assert spotify_wrangler.get_item_type(f"{URL_EXT}/chapters/{random_id()}") == ObjectType.CHAPTER
    ...
    with pytest.raises(RemoteObjectTypeError):
        spotify_wrangler.get_item_type([random_id(), "qwertyuiopASDFGHJKLZ12"])
...
def test_validate_item_type(spotify_wrangler: SpotifyDataWrangler):
    ...
    assert spotify_wrangler.validate_item_type(f"spotify:artist:{random_id()}", kind=ObjectType.ARTIST) is None
    assert spotify_wrangler.validate_item_type("spotify:user:ausername", kind=ObjectType.USER) is None
    assert spotify_wrangler.validate_item_type(f"{URL_API}/show/{random_id()}/episodes", kind=ObjectType.SHOW) is None
    assert spotify_wrangler.validate_item_type(f"spotify:episode:{random_id()}", kind=ObjectType.EPISODE) is None
    assert spotify_wrangler.validate_item_type(
        f"{URL_EXT}/audiobook/{random_id()}/chapters", kind=ObjectType.AUDIOBOOK
    ) is None
    assert spotify_wrangler.validate_item_type(f"spotify:chapter:{random_id()}", kind=ObjectType.CHAPTER) is None
...
def test_extract_ids(spotify_wrangler: SpotifyDataWrangler):
    ...
    assert spotify_wrangler.extract_ids(value) == ["8188181818181818129321"]
    value = f"{URL_EXT}/playlist/bnmhjkyuidfgertsdfertw/followers"
    assert spotify_wrangler.extract_ids(value) == ["bnmhjkyuidfgertsdfertw"]
