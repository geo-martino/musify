...
class SpotifyMock(Mocker):
    ...
    @property
    def item_type_map(self) -> dict[RemoteItemType, list[dict[str, Any]]]:
        """Map of :py:class:`RemoteItemType` to the mocked items mapped as {``id``: <item>}"""
        return {
            RemoteItemType.PLAYLIST: self.playlists,
            RemoteItemType.ALBUM: self.albums,
            RemoteItemType.TRACK: self.tracks,
            RemoteItemType.ARTIST: self.artists,
            RemoteItemType.USER: self.users,
        }

    @property
    def item_type_map_user(self) -> dict[RemoteItemType, list[dict[str, Any]]]:
        """Map of :py:class:`RemoteItemType` to the mocked user items mapped as {``id``: <item>}"""
        return {
            RemoteItemType.PLAYLIST: self.user_playlists,
            RemoteItemType.ALBUM: self.user_albums,
            RemoteItemType.TRACK: self.user_tracks,
            RemoteItemType.ARTIST: self.user_artists,
        }
    ...
    def __init__(self, **kwargs):
        ...
        self.setup_items_response(kind=RemoteItemType.PLAYLIST, id_map=playlists_map, batchable=False)
        for id_, item in playlists_map.items():
            self.setup_items_block_response_from_generator(
                kind=RemoteItemType.PLAYLIST, id_=id_, item=item, generator=self.generate_playlist_tracks
            )

        albums_map = {item["id"]: item for item in self.albums}
        self.setup_items_response(kind=RemoteItemType.ALBUM, id_map=albums_map)
        for id_, item in albums_map.items():
            self.setup_items_block_response_from_generator(
                kind=RemoteItemType.ALBUM, id_=id_, item=item, generator=self.generate_album_tracks
            )

        self.setup_items_response(kind=RemoteItemType.TRACK, id_map={item["id"]: item for item in self.tracks})
        ...
        self.setup_items_response(kind=RemoteItemType.ARTIST, id_map={item["id"]: item for item in self.artists})
        self.setup_items_response(
            kind=RemoteItemType.USER, id_map={item["id"]: item for item in self.users}, batchable=False
        )
        ...
        self.user_tracks = [self.format_user_item(deepcopy(item), RemoteItemType.TRACK) for item in self.tracks]
        self.user_albums = [self.format_user_item(deepcopy(item), RemoteItemType.ALBUM) for item in self.albums]
    ...
    def setup_search_response(self):
        ...
        def response_getter(req: Request, _: Context) -> dict[str, Any]:
            ...
            results = {}
            for kind in kinds:
                values = self.item_type_map[RemoteItemType.from_name(kind)[0]]
    ...
    def setup_items_response(
            self, kind: RemoteItemType | str, id_map: dict[str, dict[str, Any]], batchable: bool = True
    ) -> None:
        """
        ...
        url = f"{URL_API}/{kind.name.casefold()}s" if isinstance(kind, RemoteItemType) else f"{URL_API}/{kind}"
        if batchable:
    ...
    @staticmethod
    def format_user_item(response: dict[str, Any], kind: RemoteItemType) -> dict[str, Any]:
    ...
    def generate_playlist_tracks(cls, response: dict[str, Any], count: int = 0) -> list[dict[str, Any]]:
        ...
        for _ in range(count):
            ...
            track = cls.format_user_item(response=track, kind=RemoteItemType.TRACK)
