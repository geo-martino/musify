...
class TestHasPlaylists(MusifyResourceTester):
    ...
    def test_get_playlists_map_from_merge_input(self, model: HasPlaylists) -> None:
    ...
    def test_merge_playlists(self, model: HasPlaylists, playlists: list[Playlist]) -> None:
        initial, other, overlap = split_list(playlists, 2, 6)
        model = HasPlaylists(playlists=initial)
