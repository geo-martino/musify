...
def test_match_on_paths_and_any_comparers():
    ...
    assert match_result.include == config.tracks_include_reduced
    assert match_result.exclude == config.tracks_exclude
    assert sorted(match_result.compare, key=config.sort_key) == sorted(config.tracks_album, key=config.sort_key)
...
def test_match_on_paths_and_all_comparers():
    ...
    assert match_result.include == config.tracks_include_reduced
    assert match_result.exclude == config.tracks_exclude
    assert sorted(match_result.compare, key=config.sort_key) == sorted(config.tracks_artist, key=config.sort_key)
