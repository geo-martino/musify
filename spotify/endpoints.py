...
class Endpoints:
    ...
    def query(self, query: str, kind: str, limit: int = 10, **kwargs) -> list:
        ...
        :param kind: str, default=None. Examples: 'album', 'track', 'artist'.
            Refer to Spotify API for other types.
    ...
    def get_items(self, items: list, kind: str = None, limit: int = 50,
        ...
        :param add_analysis: bool, default=False. If tracks are given, search for and add analysis.
        ...
        while r['next']:
            ...
            for playlist in r['items']:
                ...
                if names is not None and name not in names:
                    # filter to only given playlist names
                    continue
    ...
    def add_to_playlist(self, playlist: str, tracks: list, limit: int = 50,
                        skip_dupes: bool = True, **kwargs) -> str:
        """
        ...
        if len(str(tracks[0]).split(':')) != 3:  # reformat tracks to URIs
            tracks = [self.convert(track, get='uri', kind='track', **kwargs)
    ...
    def clear_from_playlist(self, playlist: list, tracks_list: list = None,
                            limit: int = 100, dry_run: bool = True, **kwargs) -> str:
        """
        ...
        if tracks_list is not None:
            tracks_list = [self.convert(track, get='uri', kind="track", **kwargs)
